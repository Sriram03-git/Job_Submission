...
function renderApplications(apps) {
  ...
  apps.forEach(app => {
    ...
    row.appendChild(createCell(`<span class="status-cell-${app.status.replace(/[^a-zA-Z]/g, '')}">${app.status}</span>`));
    row.appendChild(createCell(new Date(app.applicationTimestamp).toLocaleString()));
    ...
    interviewBtn.onclick = () => updateApplicationStatus(app.id, "Interview", app.name);
    ...
    selectBtn.onclick = () => updateApplicationStatus(app.id, "Offer", app.name);
    ...
    rejectBtn.onclick = () => updateApplicationStatus(app.id, "Rejected", app.name);
...
async function loadApplications() {
  applicationsListBody.innerHTML = "<tr><td colspan='8'>Loading applications...</td></tr>";
  try {
